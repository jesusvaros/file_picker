...
export function ResourceList({
 ...
 if(isPending) return <div className="rounded border p-4">Loading resources…</div>
 if(error) return <div className="rounded border p-4 text-red-600">Error: {String((error as Error)?.message ?? error)}</div>
  ...
  return (
    <div className="rounded border">
        <ul className="divide-y">
          ...
          {!items.length && <li className="p-4 opacity-70"> No resources found</li>}
        </ul>  
    </div>
  );
}
